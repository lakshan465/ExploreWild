...
public class AdminController extends User implements Initializable {
    ...
    //add task start
    public void addTask() throws SQLException {//insert new animal information to database

        deleteTaskWhileAdding();
    ...
    public void deleteTaskWhileAdding() throws SQLException {
    ...
    public void changePwdCuz() {
        ...
        if ((cuz_id_chng_pwd.getText().isEmpty() || cuz_pwd_chng_pwd.getText().isEmpty()) && (keeper_id_chng_pwd.getText().isEmpty() || keeper_pwd_chng_pwd.getText().isEmpty())) {
            ...
            System.out.println("enter to change pwd keeper");
            //changes
            alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setTitle("confirmation massage");
            ...
            if (option.get().equals(ButtonType.OK)) {
                String pwd = LoginController.getHashPwd(keeper_pwd_chng_pwd.getText());
                System.out.println(pwd);
                ...
                try {
                    st = concpwd.prepareStatement(sql);
                    st.setString(1, pwd);
                    st.setString(2, keeper_id_chng_pwd.getText());
